...
## Data Flow Example: Displaying Graphs Comparing LogEvent and PlannedEvent, and AI Feedback on the Report Screen

When the app launches, it loads LogEvent (time records) and PlannedEvent (ideal day) from the local database. This data is combined and displayed to the user.

When the user presses the button to receive AI feedback, a prompt is constructed using LogEvent (time records) and PlannedEvent (ideal day) and posted to the Gemini API. The returned response is received and displayed on the screen.
...
The Data Layer is composed of Repositories, and each Repository has its own model. For example, LogEventRepository has LogEventModel, and PlannedEventRepository has PlannedEventModel.
...
Reading of the application's primary data (such as LogEvent and PlannedEvent) is exposed as data streams. This means that each client of the Repository must be prepared to respond to data changes.
...
A list of LogEvents can be obtained by subscribing to the LogEventRepository::getLogEventsStream flow, which emits List<LogEvent>.

Whenever the list of LogEvents changes (for example, when a new record is added), an updated List<LogEvent> is emitted to the stream.
...
Simply calling LogEventRepository.startTimer with a category ID saves a new LogEvent record to the local database.

### Data Sources

Repositories may depend on one or more data sources. For example, PlannedEventRepository depends on the following data sources:

| Name | Implementation Technology | Purpose |
|------|--------------------------|---------|
| PlannedEventDao | SQLDelight | Persistent relational data related to PlannedEvent |
...
The display of LogEvent and PlannedEvent on the timeline screen is modeled using TimelineUiState. This is a sealed interface that creates the following state hierarchy:

- Loading: Indicates data is being loaded
- Success: Indicates data has been loaded successfully. The Success state includes lists of LogEvent and PlannedEvent
...
**Example: Displaying LogEvent and PlannedEvent**

TimelineViewModel exposes uiState as StateFlow<TimelineUiState>. This hot flow is created by combining cold flows provided by LogEventRepository and PlannedEventRepository. Whenever new data is emitted, it is converted to a TimelineUiState.Success state and exposed to the UI.
...
TimelineScreen receives a lambda expression called startTimer provided by TimelineViewModel.onStartTimer. Each time the user taps the timer start button, this method is called. The ViewModel processes this action and notifies LogEventRepository.
